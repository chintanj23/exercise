...
class pagination {
	...
	protected $options = array(
		...
		'max_links_between_ellipses' => 1,
		'max_links_outside_ellipses' => 0,
	...
	public function run($current_page = 1, $query = '', $options = null) {
		GLOBAL $_DBC_REPLICA;
		$pdo_page = new PdoOpt($_DBC_REPLICA);
		$pdo_page->initiateConnection();
		$options['db_handle'] = $pdo_page->dbh;
		/*
	...
	protected function build_links() {
		...
		{	
			...
			$start = ($this->current_page -1) * ($this->options['results_per_page']) + 1;
		}
		else
		{
			$end = $this->current_page * $this->options['results_per_page'];
			$start = $end - ($this->options['results_per_page']-1);
		}
		
		$this->links_html = '<div class="table-footer-row">'. PHP_EOL;
		$this->links_html .= ' <div class="">'. PHP_EOL;
		$this->links_html .= '	<div class="float-left">'. PHP_EOL;
		$this->links_html .= '		<div class="dataTables_info"> '.$start.' to '.$end.' of '.$this->total_results.' Records</div>'. PHP_EOL;
		$this->links_html .= '	</div>'. PHP_EOL;
		$this->links_html .= '	<div class="float-right">'. PHP_EOL;
		$this->links_html .= '		<div class="dataTables_paginate">'. PHP_EOL;
	...
	protected function build_links_first_prev() {
		...
		if ($this->options['show_links_first_last'] == true) {
			if ($this->current_page == 1 && $this->options['show_links_first_last_if_dead'] == true) {
				$this->links_html .= '<li class="page-item prev ' . $this->options['class_dead_links'] . '"><a class="page-link" href="javascript:void(0);" aria-label="Previous"><span aria-hidden="true">' . $this->options['text_first'] . '</span> <span class="sr-only">Previous</span></a></li>' . PHP_EOL;
			} elseif ($this->current_page != 1) {
				$this->links_html .= '<li class="page-item ' . $this->options['class_live_links'] . '"><a href="' . $this->build_link_url(1) . '" class="page-link" ><span aria-hidden="true">' . $this->options['text_first'] . '</span><span class="sr-only">Previous</span></a></li>' . PHP_EOL;
			}
		...
		if ($this->options['show_links_prev_next'] == true) {
			if ($this->current_page == 1 && $this->options['show_links_prev_next_if_dead'] == true) {
				$this->links_html .= '<li class="page-item prev ' . $this->options['class_dead_links'] . '"><a class="page-link" href="javascript:void(0);" aria-label="Previous"><span aria-hidden="true">' . $this->options['text_prev'] . '</span><span class="sr-only">Previous</span></a></li>' . PHP_EOL;
			} elseif ($this->current_page != 1) {
				$this->links_html .= '<li class="' . $this->options['class_live_links'] . '"><a href="' . $this->build_link_url($this->current_page - 1) . '" class="page-link"><span aria-hidden="true">' . $this->options['text_prev'] . '</span><span class="sr-only">Previous</span></a></a></li>' . PHP_EOL;
			}
	...
	protected function loop_through_links($start, $finish, $array_block_label = '') {
		...
		while ($counter <= $finish) {
			if ($this->options['current_page_is_link'] == false && $counter == $this->current_page) {
				$this->links_html .= '<li class="page-item ' . $this->get_current_or_normal_class($counter) . '">' . $counter . '</li>' . PHP_EOL;
			} else {
				$this->links_html .= '<li class="page-item ' . $this->get_current_or_normal_class($counter) . '"><a href="' . $this->build_link_url($counter) . '" class="page-link">' . $counter . '</a></li>' . PHP_EOL;
			}
	...
	protected function build_links_next_last() {
		...
		if ($this->options['show_links_prev_next'] == true) {
			if ($this->current_page == $this->total_pages && $this->options['show_links_prev_next_if_dead'] == true) {
				$this->links_html .= '<li class="page-item ' . $this->options['class_dead_links'] . '"><a class="page-link" href="javascript:void(0);" aria-label="Next"><span>' . $this->options['text_next'] . '</span></a><span class="sr-only">Next</span></li>' . PHP_EOL;
			} elseif ($this->current_page != $this->total_pages) {
				$this->links_html .= '<li class="page-item ' . $this->options['class_live_links'] . '"><a href="' . $this->build_link_url($this->current_page + 1) . '" class="page-link" > <span aria-hidden="true" >' . $this->options['text_next'] . '</span><span class="sr-only">Next</span></a></li>' . PHP_EOL;
			}
		...
		if ($this->options['show_links_first_last'] == true) {
			if ($this->current_page == $this->total_pages && $this->options['show_links_first_last_if_dead'] == true) {
				$this->links_html .= '<li class="page-item ' . $this->options['class_dead_links'] . '"><a class="page-link" href="javascript:void(0);" aria-label="Next"><span aria-hidden="true" >' . $this->options['text_last'] . '</span><span class="sr-only">Next</span></a></li>' . PHP_EOL;
			} elseif ($this->current_page != $this->total_pages) {
				$this->links_html .= '<li class="page-item ' . $this->options['class_live_links'] . '"><a href="' . $this->build_link_url($this->total_pages) . '" class="page-link" > 	<span aria-hidden="true" >' . $this->options['text_last'] . '</span><span class="sr-only">Next</span></a></li>' . PHP_EOL;
			}
